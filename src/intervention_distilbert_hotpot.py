...
class DistilBertHotpotExperiment:
    ...
    def intervene(self, model, tokenizer, dataset, args, logger):
        ...
        for i in tqdm(range(0, dataset_size, args.batch_size)):
            ...
            # 构造输入 prompt
            input_texts = [f"{question} The answer is {tokenizer.mask_token}" for question, _ in batch]
            ...
            gold_answers = [answer for _, answer in batch]
            ...
            for j in range(my_batch_size):
                ...
                logprob_results = ContextAnswerLogProb(total_log_prob=None,
                                                      answer_log_prob=predicted_logprob[j].max().item(),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process Arguments for experiments with DistilBERT LLM on HotPotQA')
    ...
    for dp in dataset:
        ...
        answer = "yes" if dp["answer"] == "no" else "no"  # 示例替换逻辑，根据实际数据调整
    ...
    experiment = DistilBertHotpotExperiment(save_dir=save_dir, logger=logger)
    logger.log("=" * 50)
