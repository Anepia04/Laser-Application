...
from laser.distilbert_laser import DistilBERTLaser
from laser.gpt2_laser import GPT2Laser
...
class LaserWrapper:
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        if type(model) == LlamaForCausalLM:
        ...
        elif type(model) == DistilBertForMaskedLM:
            
            logger.log("Editing a DistilBertForMaskedLM Model")
            return DistilBERTLaser.get_edited_model(model=model,
                                              lname=lname,
                                              lnum=lnum,
                                              rate=rate,
                                              intervention=intervention,
            ...
            logger.log("Editing a GPT2LMHeadModel Model")
            return GPT2Laser.get_edited_model(
                model=model,
                lname=lname,
                lnum=lnum,
                rate=rate,
