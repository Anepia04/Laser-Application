...
class DistilBertBiosProfessionExperiment:
    ...
    def intervene(self, model, tokenizer, dataset, args, logger):
        ...
        occupations = BiasBiosOccupation.occupations
        occupation_token_ids = []

        for occ in occupations:
            tokenized = tokenizer(f" {occ.strip()}", add_special_tokens=False)["input_ids"]
            assert len(tokenized) == 1, f"Expected single token for profession: {occ}"
            occupation_token_ids.append(int(tokenized[0]))

        num_occupations = len(occupations)
        occupation_tensor = torch.LongTensor(occupation_token_ids).to(self.device)
        ...
        for i in tqdm(range(0, dataset_size, args.batch_size)):
            ...
            for text in input_texts:
                ...
                if len(tokens) > 490:
                    tokens = tokens[-490:]  # 保留后 490 个 token
                decoded_text = tokenizer.decode(tokens, skip_special_tokens=True)
                truncated_input_texts.append(f"{decoded_text} {tokenizer.mask_token}")
            ...
            gold_answers = [BiasBiosOccupation.occupations[answer_ix] for _, answer_ix in batch]
            ...
            # 收集每个样本中各个职业的 log prob
            choices_token_logprobs = []
            for occ_token_id in occupation_token_ids:
                batch_choice_token_ids = torch.LongTensor([occ_token_id] * my_batch_size).unsqueeze(1).to(self.device)
            ...
            # 记录结果
            for j in range(my_batch_size):
                pred_occ = occupations[predicted_occupation_ix[j]]
                true_answer = gold_answers[j]
                is_correct = pred_occ.lower().strip() == true_answer.lower().strip()
                answer_len = 1
                logprob_results = ContextAnswerLogProb(total_log_prob=None,
                                                      answer_log_prob=choices_token_logprobs[predicted_occupation_ix[j], j],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process Arguments for experiments with DistilBERT LLM on Bios-Profession')
    ...
    parser.add_argument('--dtpts', type=int, default=19223, help='# samples per instruction')
    ...
    parser.add_argument('--home_dir', type=str,
                        default="/mnt/data/iclr2024/bios_profession/distilbert_results",
    ...
    for dp in dataset:
        ...
        # 确保 answer_ix 是整数类型
        assert isinstance(answer_ix, int), f"Expected answer_ix to be int, got {type(answer_ix)}"
        assert 0 <= answer_ix < len(BiasBiosOccupation.occupations), f"Index out of range: {answer_ix}"

        # 构造 prompt 并保留 mask token
        if bio.strip().endswith(".") or bio.strip().endswith("?"):
            prompted_bio = f"Consider the following text: {bio.strip()} The profession of the person in this text is called {tokenizer.mask_token}."
        else:
            prompted_bio = f"Consider the following text: {bio.strip()}. The profession of the person in this text is called {tokenizer.mask_token}."

        # 保留 answer_ix 为整数用于模型训练和评估
        processed_data.append((prompted_bio, answer_ix))

    experiment = DistilBertBiosProfessionExperiment(save_dir=save_dir, logger=logger)
    logger.log("=" * 50)
